...
def pose_estimation(frame, aruco_dict_type, matrix_coefficients, distortion_coefficients, dictionary, arucoParams):
    ...
    if len(corners) > 0:
        for i in range(0, len(ids)):
            ...
            distance = round(tvec[i][0][2] * 100, 1)
            ...
            print("  x-axis: {:.2f}".format(theta_x))
            print("  y-axis: {:.2f}".format(theta_y))
            print("  z-axis: {:.2f}".format(theta_z))

            move_distance = math.sin(math.radians(abs(theta_y))) * distance

            print("move_distance: ", move_distance)

            if(10 < int(theta_y) < 90):
                drone.rotate_clockwise(abs(int(theta_y)))
            elif (-90 < int(theta_y) < -10):
                drone.rotate_counter_clockwise(abs(int(theta_y)))

            # speed = 10, x = move_distance, time = x / 10
            target_time = move_distance / 10
            ...
            # drone.send_command('cw ', -1 * theta_y)
